...
inline bool cartesian_dda::xCanStep(long current, long target, bool dir)
{
  ...
  //so, if x min endstop is inverting, 1 = open, 0 = closed
  //so analogRead() > Thresh == 1 == open, analogRead() < THRESH == 0 == closed
#if X_ENDSTOP_INVERTING
  //  if(!dir && !digitalRead(X_MIN_PIN) )
  if(!dir && analogRead(X_MIN_PIN) < 60)
    return false;
#else
  //if non-inverting-- not the case for me
  if(!dir && digitalRead(X_MIN_PIN) )
    return false;
#endif
#endif
  ...
  if(dir && analogRead(X_MAX_PIN) < 60)
    return false;
#else
  if(!dir && digitalRead(X_MAX_PIN) )
    return false;
...
inline bool cartesian_dda::yCanStep(long current, long target, bool dir)
{
  ...
  if(!dir && analogRead(Y_MIN_PIN) < 60)
    return false;
#else
  if(!dir && digitalRead(Y_MIN_PIN) )
    return false;
  ...
  if(dir && analogRead(Y_MAX_PIN) < 60)
    return false;
#else
  if(!dir && digitalRead(Y_MAX_PIN) )
    return false;
